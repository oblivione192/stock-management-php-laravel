...
const load = async (): Promise<void> => {
    ...
    try {
        if (!dashboardStore.getHasFetched) {
            data.value = await getJson<DashboardResponse>(
                '/inventory/dashboard',
            );
        ...
        <template v-else-if="data">
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    ...
                    <p class="text-2xl font-semibold">
                        {{ currency(data.metrics.totalStockValue) }}
                    </p>
                </article>
            </div>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                >
                    <h3
                        class="mb-3 text-sm font-semibold tracking-wide text-neutral-300 uppercase"
                    >
                        Low-stock products
                    </h3>
                    <ul class="space-y-2 text-sm">
                        ...
                        >
                            ...
                            <p class="text-neutral-400">
                                Current: {{ item.current_stock }} | Reorder:
                                {{ item.reorder_level }}
                            </p>
                        </li>
                        <li
                ...
                >
                    ...
                    <ul class="space-y-2 text-sm">
                        <li
                            v-for="item in data.recentMovements"
                            :key="item.id"
                            class="rounded-md border border-neutral-800 p-3"
                        >
                            <p class="font-medium">
                            ...
                            <p class="text-neutral-400">
                                {{ item.movement_type }} | Qty:
                                {{ item.quantity }} |
                                {{ item.reference ?? 'No reference' }}
                            </p>
                        </li>
                        <li
                            v-if="data.recentMovements.length === 0"
                            class="text-neutral-400"
                        >
                            No movement data.
                        </li>
